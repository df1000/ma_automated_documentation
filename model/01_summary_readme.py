...
def check_repo_processed(repo_owner, repo_name):
    loaded_json = open_json(path='../data/helper/repos_processed.txt')
...
for i in repo_list:
    ...
    if not check_repo_processed(repo_owner=i[0], repo_name=i[1]): 
        ...
        repo_data = open_json(path=f'../data/input_data/{i}.json')
        repo_name = i[1]
        repo_owner = i[0]
        num_of_chars = i[2]
        ...
        if num_of_tokens < 127000:
            ...
            summary, summary_tokens = send_query(prompt=prompt_summary)
            ...
            readme, readme_tokens = send_query(prompt=prompt_readme)

            write_json(repo_owner=repo_owner, repo_name=repo_name, summary_list=summary_list)
            write_preprocessed_repo(repo_owner=repo_owner, repo_name=repo_name)

            print(f'Files for repository: {repo_name} from {repo_owner} successfully created.')
            ...
            for prompt in sub_prompts:
                ...
                sub_summary, sub_summary_tokens = send_query(prompt_summary)
